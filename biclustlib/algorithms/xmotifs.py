...
class ConservedGeneExpressionMotifs(BaseBiclusteringAlgorithm):
    ...
    def run(self, data):
        ...
        data : numpy.ndarray
        """
        data = check_array(data, dtype=np.int64, copy=True)
    ...
    def _find_motif(self, data, row_indices):
        ...
        best_motif = Bicluster(np.array([], np.int64), np.array([], np.int64))
        ...
        for s in seeds:
            ...
            for i in range(self.num_sets):
                ...
                selected_rows = np.array([y for x, y in enumerate(row_indices) if np.all(rows_comp_data[x])], np.int64)
